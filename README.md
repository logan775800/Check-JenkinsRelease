...
| 2. 部署 | `admin_ALL` 视图里的 job（`ADMIN_JENKINS_DEPLOY_VIEW`） | **按站点比「它装的是哪一版」**：见下 |

部署 job 自己没有版本参数，它装的是 `build_admin` 当时产出的那份制品。所以判定方式是：
**用部署时刻往前找最近一次成功的 `build_admin`，那次的版本就是这个站点的实际版本**，
再拿它和发布单比。

这样能抓到一种否则必然漏掉的情况：**某个站点的部署跑在 `build_admin` 这次构建之前** ——
它装的是上一版制品，而它自己的部署 job 是**绿色成功**的。不比时间的话报告全绿，
线上却有站点还是旧后台。和「分支名一样但代码不一样」是同一类坑，只是这里能精确判定。

同理：`build_admin` 打错了版本，装了它的**每个站点都会一起判「版本不符」** ——
它们确实都是错的版本，不该只红构建那一行。

**部署 job 是按站点的**：实测 `admin_ALL` 视图里 70 个 job 全是
`AR{编号}-admin-{国家}-{站点名}` 命名，和 AR 那套规约一致。所以页面上后台部署
**和别的组件一样按站点铺开**，发布单列了哪个站点就查哪个站点的 admin 发了没有 ——
70 个站点手工一个个点，漏一个太正常，这正是要抓的。

还会抓两种情况：

- **`build_admin` 构建成功了，但一个站点的部署都没执行**（制品打好了忘了去点 Build Now）；
- **后台上有、但这次发布单没列的站点**，会单独说明「未核对」——「这次不发」和
  「漏发」是两回事，只有你自己知道是哪种，工具不替你判。
...
`python -m py_compile` 只管 Python，查不到里面的 JS。它做两关：

1. **括号/引号平衡** —— 少一个引号整页 JS 直接死；
2. **用了但没声明的标识符** —— 抓 `st is not defined` 这类。

第二关是踩出来才加的：一处字符串替换静默没生效，`const st=...` 没进去而 `+st+`
进去了，**平衡校验全绿**，页面却白屏只留一句 "st is not defined"。
平衡校验对这种错完全无能为力，必须单独一关。
两关都用同一个扫描器产出的代码骨架 —— 分开写会漏掉正则字面量里的引号，
骨架从那里整段错位（第一版就是这样冒出几十个误报的）。

⚠️ **改内联 JS 时，别用脚本做字符串替换**：这台机器的 bash heredoc 会把 `\n`
折成真换行，替换悄悄匹配不上、又不报错，你以为改了其实没改。用编辑器改，
或者改完 grep 一遍确认新内容真的进去了。
